...
class GitHubReleaseManager:
    ...
    def get_version_file_name(self) -> str:
        ...
        return (
            f".version_{self.project_name}_{self.branch}.json"
            if self.project_name
            else f".version_{self.branch}.json"
        )
    ...
    def get_release_name(self, version: str) -> str:
        ...
        return (
            f"Release {self.project_name} {version}"
            if self.project_name
            else f"Release {version}"
        )

    def get_version_file_content(self) -> Dict:
        """Get and parse version file content from local file.

        Reads the local version file written by update_version.py rather than
        fetching from the GitHub API, to avoid race conditions with API caching
        after commit_files.py commits the file.
        """
        version_file = self.get_version_file_name()

        # Read from local file (written by update_version.py in the same workflow)
        try:
            with open(version_file, "r") as f:
                data = json.load(f)
            logger.info(f"Read version data from local file: {version_file}")
            return data
        except FileNotFoundError:
            raise GitHubAPIError(f"Version file {version_file} not found locally")
        except json.JSONDecodeError as e:
            raise GitHubAPIError(
                f"Failed to parse local version file {version_file}: {str(e)}"
            )
    ...
    def create_tag(self, tag_name: str) -> bool:
        ...
        if tag_exists:
            ...
            if not self.delete_release(tag_name) or not self.delete_tag(tag_name):
                raise GitHubAPIError(
                    f"Failed to delete existing tag/release: {tag_name}"
                )

        response = self._make_request(
            "POST", "git/refs", data={"ref": f"refs/tags/{tag_name}", "sha": self.sha}
        )
        return True
