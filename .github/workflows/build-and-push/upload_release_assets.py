...
def upload_release_assets(version: str, image_name: str):
    ...
    try:
        ...
        logger.info(
            f"Uploading release assets for version {versioned_tag} with command: {upload_cmd}"
        )
...
def main():
    ...
    # Upload release assets (let exceptions propagate to retry_main)
    ...
    def retry_main(max_retries=3, delay=2):
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Retrying due to error: {e} (attempt {attempt + 1}/{max_retries})"
                )
                time.sleep(delay)
        else:
